...
const hsts = source.match(/key:\s*"Strict-Transport-Security"[\s\S]{0,200}?value:\s*"([^"]+)"/i)?.[1] ?? "";
...
check("HSTS protects subdomains from transport downgrade", /(?:^|;)\s*includeSubDomains(?:\s*;|\s*$)/i.test(hsts), hsts);
